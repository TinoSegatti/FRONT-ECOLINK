...
import { GestionableFieldKey, Cliente } from '../types';
import { Categoria } from '../services/api/clientes';
...
interface CategoriasGestionablesProps {
  ...
  handleEditarCategoria: (field: GestionableFieldKey, oldValor: string, newValor: string, color?: string) => Promise<Categoria | void>;
  handleEliminarCategoria: (field: GestionableFieldKey, valor: string) => Promise<Categoria | void>;
  error?: string;
...
export default function CategoriasGestionables({
  ...
  const handleSave = async () => {
    try {
      ...
      console.error(`Error processing category for ${field}:`, err);
      // Errores se manejan en el formulario principal
    }
  };
  ...
  const handleDelete = async (valor: string) => {
    try {
      ...
      console.error(`Error deleting category for ${field}:`, err);
      // Errores se manejan en el formulario principal
    }
  };
  ...
  return (
    <>
      <Form.Group controlId={field}>
        ...
        {error && (
          <div className="text-danger" style={{ fontSize: '0.875rem', marginBottom: '0.25rem' }}>
            {error}
          </div>
        )}
        <InputGroup>
