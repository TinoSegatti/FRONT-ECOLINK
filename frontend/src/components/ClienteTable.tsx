...
export default function ClienteTable({
  ...
  // Cargar colores de categorías (ahora como clases CSS)
  useEffect(() => {
      ...
      try {
        const promises = columnasFiltrables.map(async (campo) => {
          ...
          const colorMap = categorias.reduce(
            (acc, cat) => {
              // Generar nombre de clase CSS normalizado
              const className = `category-${campo}-${String(cat.valor)
                .toLowerCase()
                .replace(/\s+/g, "-")
                .replace(/[^a-z0-9-]/g, "")}`
              acc[cat.valor] = className
              return acc
            },
        ...
        <table className="table table-striped table-bordered mt-4 table-sm table-custom">
          <thead>
            <tr>
              ...
              <th className="sticky-actions small">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {(sortedClientes || []).map((cliente) => (
              <tr key={cliente.id}>
                {displayedColumns.map((columna) => {
                  ...
                  const className = esColumnaGestionable
                      ...
                      className={
                        columna === "nombre"
                          ...
                          : [
                            ...
                            ? `small single-line-column ${className}`
                            : `small ${className}`
                      }
                      title={
                        [
